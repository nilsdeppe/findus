...
#include "findus/Serialize/View.hpp"
...
TEST_CASE("Serialize.View") {
  ...
  findus::serialize::View<int> view1(data.data(), 3);
  ...
  findus::serialize::View<int> view2(data.data(), static_cast<size_t>(3));
  CHECK(view2.item_ == data.data());
  CHECK(view2.number_of_items_ == 3);

  // Construct with reference to single item
  findus::serialize::View<int> view3(data[1]);
  ...
  findus::serialize::View<ComplexType> complex_view(complex_data.data(), 2);
  ...
  findus::serialize::View<ComplexType> single_complex_view(complex_data[0]);
